...
from utils.data_transform import load_and_transform_thermal_data

if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    parser = argparse.ArgumentParser("")
    parser.add_argument("--pretrain_weights", type=str, default='./ckpts/pretrained_weights.pt', required=False)
    parser.add_argument("--modality", type=str, default='thermal', required=False)
    parser.add_argument("--centre_embeddings_path", type=str, default='./centre_embs/thermal_llvip_center_embeddings.pkl', required=False)
    ...
    images = ["./assets/thermal_people.jpg", "./assets/thermal_background.jpg"]
    inputs = {
        ModalityType.THERMAL: load_and_transform_thermal_data(images, device),
    }
